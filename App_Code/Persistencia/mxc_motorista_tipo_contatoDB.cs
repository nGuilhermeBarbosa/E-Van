...
public class mxc_motorista_tipo_contatoDB
{
    ...
    public static DataSet SelectAll(int id)
    {
        ...
        string sql = "select mxc_descricao, tdo_image from usu_usuario usu inner join mot_motorista mot on mot.usu_id = usu.usu_id inner join doc_documento doc on doc.mot_id = mot.mot_id inner join tdo_tipodocumento tdo on tdo.tdo_id = doc.tdo_id inner join pes_pessoa pes on pes.pes_id = usu.pes_id inner join mxc_motorista_tipo_contato mxc on mot.mot_id = mxc.mot_id inner join tpc_tipo_contato tpc on tpc.tpc_id = mxc.tpc_id where usu.usu_id = ?id ";
